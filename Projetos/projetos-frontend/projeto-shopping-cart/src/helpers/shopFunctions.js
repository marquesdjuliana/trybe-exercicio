 ...
 */
// Função que adiciona produto a cart:
const addProductCart = (elementBTN, id) => {
  elementBTN.addEventListener('click', async () => {
    saveCartID(id);
    const productId = await fetchProduct(id);
    const cart = document.querySelector('.cart__products');
    const addElementRefProduct = createCartProductElement(productId);
    cart.appendChild(addElementRefProduct);
  });
};
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  addProductCart(cartButton, id); // chamando a função com BTN como parâmetro;
  return section;
};
